class Pokemon
  ...
  def self.find(number,database_connection)
    ...
    Pokemon.new(id:pokemon_array[0],name:pokemon_array[1],type:pokemon_array[2],db:database_connection,hp:pokemon_array[0])
  end
  ...
  def alter_hp(new_hp,database_connection)
    ...
    database_connection.execute("UPDATE pokemon SET hp=#{15} WHERE id=#{1}")
  end
